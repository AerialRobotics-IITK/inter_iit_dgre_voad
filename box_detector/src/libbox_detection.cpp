...
void BoxDetector::setCannyParams(const int& lower, const int& upper, const int& size) {
...
void BoxDetector::findGoodContours() {
...
void BoxDetector::fitRect(cv::Mat& board) {
    int area_curr = 0;
    int area_prev = 0;
    center_.x = -1;
    center_.y = -1;
...
void BoxDetector::drawContours(cv::Mat& board) {
    cv::drawContours(board, good_contours_, -1, cv::Scalar(255, 255, 255), 2);
}

std::pair<int, int> BoxDetector::getCentre() {
    return centre_;
}

cv::Mat BoxDetector::getThresh() {
    return thresh_img_;
}

double BoxDetector::getDistance() {
    return distance_;
}

double BoxDetector::getArea() {
    return area_;
}

void BoxDetector::setHSVMin(const int& h, const int& s, const int& v) {
    hsv_min_ = cv::Scalar(h, s, v);
}

void BoxDetector::setHSVMax(const int& h, const int& s, const int& v) {
    hsv_max_ = cv::Scalar(h, s, v);
}

void BoxDetector::setMinArea(const int& area) {
