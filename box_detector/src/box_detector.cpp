...
void BoxDetectorNode::init(ros::NodeHandle &nh) {
  img1_sub_ =
      nh.subscribe("image_raw_1", 1, &BoxDetectorNode::image1Callback, this);
  img2_sub_ =
      nh.subscribe("image_raw_2", 1, &BoxDetectorNode::image2Callback, this);
  ...
  global_coord_pub_ =
      nh_private.advertise<detector_msgs::GlobalCoord>("global_coord", 10);
...
// void BoxDetectorNode::cameraInfoCallback(const sensor_msgs::CameraInfo& msg)
// {
//     std::copy(msg.P.begin(), msg.P.end(),
//     std::back_inserter(camera_matrix_));
// }
...
void BoxDetectorNode::findGlobalCoordinates() {
  ...
  if (debug_) {
    ...
    // std::cout << translation_(0)<<" "<<translation_(1) <<"
    // "<<translation_(2)<<std::endl; std::cout << global_coordinates_(0) << " "
    // << global_coordinates_(1) << " " << global_coordinates_(2) << std::endl;
    // std::cout << "Required Marker Detected"
    //           << "\n";
    // std::cout << "centre_coordinates" << centre_(0) << " " << centre_(1) <<
    // std::endl; std::cout << "pose estimation"; std::cout << "rotation:" <<
    // rvecs[0][0] << " " << rvecs[0][1] << " " << rvecs[0][2] << "\n"
    //           << "translation" << tvecs[0][0] << " " << tvecs[0][1] << " " <<
    //           tvecs[0][2] << std::endl;
...
void BoxDetectorNode::image1Callback(const sensor_msgs::ImageConstPtr &msg) {
...
void BoxDetectorNode::image2Callback(const sensor_msgs::ImageConstPtr &msg) {
  cv_bridge::CvImagePtr cv_ptr_;
